...
var user2 = {
    name: 'Test User 2',
    email: 'user2@servicetest.local',
    principal: 'user2@servicetest.local',
    provider: 'local',
    password: 'Minisap'
};
...
describe('User Service', function () {
    ...
    describe('getUsers', function () {
        ...
        describe('test cases to be cleaned', function () {
            ...
            it('Show List of Users>>should retrieve users by list of ids', function (done) {
                var userService = commonServer.registry.getModule('UserService');

                userService.createUser(user2, emailTemplateData, dummyContext)
                .then(function (doc) {
                    user2 = doc;
                    var userList = [user._id, user2._id];
                    userService.showAvatarListBin(userList)
                        .then(function (doc) {
                            expect(doc).to.exist;
                            expect(doc).length.to.be(2);
                            expect(doc[0]._id.toString()).to.equal(user._id.toString());
                            expect(doc[1]._id.toString()).to.equal(user2._id.toString());
                        })
                        .then(testPassed(done), testFailed(done));
                });
            });
            ...
            it('Update Last Login', function (done) {
                var userService = commonServer.registry.getModule('UserService'),
                    firstUserUpdateTime,
                    DELAY = 20,
                    USERID = user._id;
                ...
                userService.updateLastLogin(USERID)
                    ...
                    .then(function retrievedUserChecks(userDoc) {
                        expect(userDoc).to.exist;
                        expect(userDoc).to.be.an('object');
                        expect(userDoc._id.toString()).to.equal(USERID.toString());
                        firstUserUpdateTime = userDoc.last_login;
                    })
                    .delay(DELAY)
                    .then(function updateUsersLastLogin() {
                        return userService.updateLastLogin(USERID);
                    })
                    .then(function refetchUser() {
                        return userService.getUserById(USERID);
                    })
                    .then(function checkLastLoginChanged(userLastLogin) {
                        expect(userLastLogin).to.exist;
                        expect(userLastLogin).to.be.an('object');
                        expect(userLastLogin._id.toString()).to.equal(USERID.toString());
                        assert.isAbove(userLastLogin.last_login.getTime(), firstUserUpdateTime.getTime() + DELAY, 'last login should be at least ' + DELAY + 'ms after the initial time');
                        assert.isBelow(userLastLogin.last_login.getTime(), firstUserUpdateTime.getTime() + 500, 'last login should be no later than 500ms after the initial time');
                    })
                    .then(testPassed(done), testFailed(done));
            });
        });
    ...
    describe('save and get Preferences', function () {
        it('save and retrieve user preferences', function (done) {
            var userService = commonServer.registry.getModule('UserService');
            userService.updatePreferences(user._id, {
                help: {disable: true},
                projectsHelp: {disable: true},
                showAdvancedProperties: {enable: true}
            })
                .then(function () {
                    userService.getPreferences(user._id).
                        then(function (preferences) {
                            expect(preferences).to.exist;
                            expect(preferences.preferences.help.disable).to.eq(true);
                            expect(preferences.preferences.projectsHelp.disable).to.eq(true);
                            expect(preferences.preferences.showAdvancedProperties.enable).to.eq(true);
                        })
                        .then(testPassed(done), testFailed(done));
                });
