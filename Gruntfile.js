...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        convert: {
            ...
            json2xml: {
                ...
                src: ['test/results/testReport.json'],
                dest: 'test/results/testResults.xml'
            }
        },
        ...
        mochaTest: require('./grunt-conf/mochaTest.js'),
    ...
    grunt.registerTask('test', function (target) {
        var tasks = {
            server: ['env:test', 'copy:server', 'mocha_istanbul'],
            client: ['env:test', 'build:test', 'karma', 'copy:testResult'],
            e2e: ['express:dev', 'wait:dev', 'protractor', 'wait:report', 'trimJsonOutput', 'wait:report', 'checkTestFailed'],
            dflt: ['env:test', 'eslint', 'test:server', 'test:client']
        };
    ...
    grunt.registerTask('dist', ['env:dev', 'build:dist', 'test', 'jsdoc:dist']);
