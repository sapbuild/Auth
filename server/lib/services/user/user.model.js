...
var UserSchema = mongoose.createSchema('auth', {
    ...
    preferences: {
        ...
        showAdvancedProperties : { enable: {type: Boolean, default: false}}
    }
});
...
UserSchema.methods.incrementLoginAttempts = function () {
    ...
    if (this.login_attempts + 1 >= MAX_LOGIN_ATTEMPTS && !this.isLocked) {
        updates.$set = {lock_until: Date.now() + 60 * 1000 * LOCK_TIME}; // lock time is in minutes Date.now in ms
    }
...
/**
 * Updates the last_login field with the current date.
 */
UserSchema.methods.updateLastLogin = function () {
    var updates = { last_login: new Date() };
 ...
 * @param  {Function} cb callback with parameters: (err, user, reason)
 */
UserSchema.statics.getAuthenticated = function (principal, password, cb) {
    var reasons = this.failedLogin,
        lcPrincipal = principal.toLowerCase();
    serviceLogger.debug('Authenticating user  ', lcPrincipal);

    this.findOne({principal: lcPrincipal}, function (err, user) {
        ...
        if (!user) {
            serviceLogger.debug('Unknown user ' + lcPrincipal);
        ...
        if (user.lock_until && user.lock_until > Date.now()) {
            if (!user.incrementLoginAttempts()) {
                serviceLogger.debug('Account locked for user ' + lcPrincipal);
        ...
        if (!isMatch) {
            if (!user.incrementLoginAttempts()) {
                serviceLogger.debug('Invalid credentials for  user ' + lcPrincipal);
                return cb(null, null, reasons.PASSWORD_INCORRECT);
            }
        }

        if (!user.updateLastLogin()) {
            serviceLogger.error('Couldn\'t update last login for user ' + lcPrincipal);
        }

        // acc. was not locked
        if (!user.login_attempts && !user.lock_until) {
            serviceLogger.debug('User ' + lcPrincipal + ' authenticated');
    ...
    .validate(function (value, respond) {
        ...
        if (this.provider !== 'local' && (!value || value.length === 0)) {
            ...
            this.constructor.findOne({email: value}, function (err, user) {
                if (err) {
                    serviceLogger.error('Email validation error');
                    throw err;
                }
                if (user) {
                    serviceLogger.debug('Email validation ok? ' + (_id === user.id));
                    return respond(_id === user.id);
                }
                respond(true);
            });
        }
