    ...
    if (context.auth) {
        ...
        return this.rawCreateUser(newUser, null, context)
                ...
                .catch(function (err) {
                    ...
                    auditService.logSystemEvent('Authentication', 'sign up failed', 'Unable to create user', {
                        email: newUser.email,
                        name: newUser.name || '',
                        error: err
                    });
...
UserService.prototype.createUser = function (newUser, emailTemplateData, context) {
    return this.rawCreateUser(newUser, emailTemplateData, context)
            ...
            .catch(function (err) {
                ...
                auditService.logSystemEvent('Authentication', 'Local sign up failed', 'Unable to create user', {
                    email: newUser.email,
                    name: newUser.name || '',
                    error: err
                });
...
UserService.prototype.rawCreateUser = function (newUser, emailTemplateData, context) {
    ...
    return Promise.resolve(accessService.getPermissions(userEmail, 'access'))
            .then(function (permissions) {
                ...
                // If coming from SSO in Admin application, set standard role by default.
                if (isAdmin) {
                    roles.push('standard');
                } else {
                    roles = accessService.getRoles('access', permissions);
                ...
                // Create the user
                return Promise.resolve(User.create(newUser))
                        .catch(function (errCreatingUser) {
                            // Catching 2 concurrent requests that called 'user create' at the same time
                            serviceLogger.warn('Error creating user');
                            if (!errCreatingUser || errCreatingUser.name !== 'ValidationError') {
                                throw new NormanError('Error creating user', errCreatingUser);
                            }
            ...
            // Send Creation notification to modules that made a request for it (eg. stored in userGlobalChangeHandlers)
            .then(function (user) {
                ...
                function nextHandler() {
                    ...
                    if (typeof handler === 'function') {
                        return Promise.resolve(handler(user._id, changeInfo, context)).then(nextHandler);
                    }
                    return Promise.resolve(handler.onUserGlobalChange(user._id, changeInfo, context)).then(nextHandler);
                }
...
UserService.prototype.getUserById = function (id) {
    ...
    User.findOne({
                _id: id,
                deleted: false
            }, 'name email provider has_email_verified avatar_url preferences last_login',
...
UserService.prototype.show = function (userId) {
    var deferred = Promise.defer(), findUserError;

    User.findById(userId, USER_FIELDS, function (err, user) {
        if (err || !user || user.deleted) {
            if (err) {
                findUserError = new NormanError(err);
            }
            else {
                findUserError = new NormanError('User not found or deleted', 404);
            }
            serviceLogger.error(findUserError);
            return deferred.reject(findUserError);
        }
        deferred.resolve(user);
    });
    return deferred.promise;
...
/**
 * Gets avatar information - returns avatar_bin instead of url
 * @param userList - list of users to find
 */
UserService.prototype.showAvatarListBin = function (userList) {
    serviceLogger.debug({
        userList: userList
    }, '>> showAvatarListBin()');
    ...
    User.find({
        ...
        deleted: false
            ...
            .exec(function (err, users) {
                ...
                serviceLogger.info('<< showAvatarListBin(), returning users');
                deferred.resolve(users);
            });
    return deferred.promise;
};
 ...
 * @param {boolean} caseInsensitiveSearch Optional parameter to perform the search in a case insensitive manner if set to true.
 * NOTE1: the regexp built will be left anchored to minimize processing.
 * NOTE2: You can pass your own regexp in 'email' and set 'caseInsensitiveSearch' to false (or undefined) to prevent the left anchoring.
 * @returns {Promise}
 */
UserService.prototype.getUserByEmail = function (email, caseInsensitiveSearch) {
    serviceLogger.info('GetUser', {email: email});
    if (caseInsensitiveSearch === true && typeof email === 'string') {
        // see https://developer.mozilla.org/en/docs/Web/JavaScript/Guide/Regular_Expressions
        var emailRegexp = email.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        // left anchor the regexp
        email = new RegExp('^' + emailRegexp, 'i');
        serviceLogger.debug('email regexp: ' + email);
    }
    return Promise.resolve(User.findOne({
        email: email,
        deleted: false
...
/**
 * Updates the last login time of a given user
 *
 * Note: in the future, the Promise could 'return' the user.
 * In that case, this method will need an additional optional parameter that will
 * be used to determine which 'columns' to retrieve, in order to support  getUserByPrincipal
 * and getUserById for instance.
 *
 * @param {ObjectId} userId The user ID.
 * @return {Promise} Returns a promise.
 */
UserService.prototype.updateLastLogin = function (userId) {
    serviceLogger.debug('UpdateLastLogin');
    return Promise.resolve(User.findOneAndUpdate(
        { _id: userId },
        { last_login: new Date() },
        { upsert: false },
        function (err) {
            if (err) {
                serviceLogger.error(err, 'Failed to update user\'s last login');
            }
            serviceLogger.debug('UpdateLastLogin completed');
        }));
};
...
UserService.prototype.updatePreferences = function (userId, preferences, context) {
    ...
    User.findById(userId, function (err, user) {
        ...
        changes.old_preferences.showAdvancedProperties = {};
        changes.new_preferences.showAdvancedProperties = {};
        if (!_.isUndefined(preferences.help.disable) && user.preferences.help.disable !== preferences.help.disable) {
        ...
        if (!_.isUndefined(preferences.projectsHelp.disable) && user.preferences.projectsHelp.disable !== preferences.projectsHelp.disable) {
        ...
        if (!_.isUndefined(preferences.showAdvancedProperties.enable) && user.preferences.showAdvancedProperties.enable !== preferences.showAdvancedProperties.enable) {

            changes.old_preferences.showAdvancedProperties.enable = user.preferences.showAdvancedProperties.enable;
            changes.new_preferences.showAdvancedProperties.enable = preferences.showAdvancedProperties.enable;
            user.preferences.showAdvancedProperties.enable = preferences.showAdvancedProperties.enable;
        }
        ...
        return results;
            ...
            .catch(function (createErrors) {
                ...
                if (allFailed) {
                    ...
                    throw new NormanError('Unable to import users', createErrors);
                }
                return results;
            });
};

UserService.prototype.upgrade_0_1_0 = function () {
    serviceLogger.debug('Running upgrade_0_1_0');
    return Promise.resolve(User.find({}).exec())
        .then(function updateFirstLoginForAllUsers(users) {
            serviceLogger.info('Found users, count: ' + users.length);
            var promises = [];
            users.forEach(function updateFirstLogin(user) {
                try {
                    serviceLogger.debug('Updating user: ' + user.email + ', last login: ' + user.last_login);
                    user.first_login = user.last_login;
                    promises.push(Promise.resolve(user.save()));
                }
                catch (err) {
                    serviceLogger.error(err, 'Failed to update first_login for user ' + user.email);
                }
            });
            return Promise.all(promises);
        });
...
UserService.prototype.upgradeSchema = function (version, done) {
