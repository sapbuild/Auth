...
PassportService.prototype.shutdown = function (done) {
                        ...
                        .then(function (user) {
                            if (user) {
                                ...
                                if (_.isEmpty(user.facebook)) {
                                    ...
                                    profile.token = token;
                                    userService.Update(user._id, { facebook: profile, last_login: new Date() })
                                        .then(function (user2) {
                                            // audit action
                        ...
                        .then(function (user) {
                            if (user) {
                                ...
                                if (_.isEmpty(user.google)) {
                                    // update facebook profile and token
                                    profile.token = token;
                                    userService.Update(user._id, { google: profile, last_login: new Date() })
                                        .then(function (user2) {
                                            // audit action
    ...
    passport.use(new LinkedInStrategy({
            ...
            process.nextTick(function () {
                ...
                userService.GetUserByEmail(profile.emails[0].value)
                    .then(function (user) {
                        if (user) {
                            ...
                            if (_.isEmpty(user.linkedin)) {
                                ...
                                profile.token = token;
                                return userService.Update(user._id, { linkedin: profile, last_login: new Date() });
                            }
                    ...
                    .then(function (user3) {
                        return userService.updateLastLogin(user3._id)
                            .finally(function () {
                                return user3;
                            });
                    })
