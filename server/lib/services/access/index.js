...
AccessService.prototype.checkSchema = function (done) {
...
AccessService.prototype.get = function (options) {
    ...
    promises.push(accessRules.find(findParam).sort(options.sort).lean().exec());
        ...
        .then(function (role) {
            ...
            if (role === topRole) {
                userService.getUserByEmail(id)
                    .then(function (user) {
                        if (user) {
                            userService.setRole(user._id.toString(), topRole, context);
                        }
                    })
                    .catch(function () {
    ...
    return Promise.resolve(accessRules.findOne({_id: id}).exec())
        .then(function (access) {
            ...
            if (!access) {
                ...
                serviceLogger.info('Nb permissions: ' + results.length + ' for: ' + id);
                return results;
            }
        ...
        .catch(function (err) {
            serviceLogger.warn(err, 'Failed to get permissions for: ' + id);
            throw err;
        });
