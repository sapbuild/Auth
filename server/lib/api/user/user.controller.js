...
/**
 * Is the user in the blacklist ?
 */
module.exports.isOptedOut = function (req, res) {
    optOutService.isOptedOut(req.params.id)
        ...
        .catch(function (err) {
            res.status(500).json(new NormanError('Unable to verify opt out', err));
        });
};
 ...
 */
module.exports.optOut = function (req, res) {
    optOutService.add(req.body.email, req.context)
        .then(function () {
            res.status(200).json({
                status: 'success'
            });
        ...
        .catch(function (err) {
            res.status(500).json(new NormanError('Could not opt out user', err));
        });
};
 ...
 */
module.exports.optIn = function (req, res) {
    optOutService.remove(req.body.email, req.context)
        .then(function () {
            res.status(200).json({
                status: 'success'
            });
        ...
        .catch(function (err) {
            res.status(500).json(new NormanError('Could not opt in user', err));
        });
};
...
module.exports.updatePreferences = function (req, res) {
    ...
    var userId = req.user._id;
    var preferences = {
        help: {disable: (req.body.help) ? req.body.help.disable : undefined},
        projectsHelp: {disable: (req.body.projectsHelp) ? req.body.projectsHelp.disable : undefined},
        showAdvancedProperties: {enable: (req.body.showAdvancedProperties) ? req.body.showAdvancedProperties.enable : undefined}
    };
