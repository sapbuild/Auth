        ...
        if (req.connection.authorized) {
            if (isTrusted(clientCert, trust)) {
                ...
                try {
                    ...
                    getAuditService().logSystemEvent('Authentication', 'signup', 'authentication is successful', { principalName: principalName });

                    getUserService().getUserByPrincipal(principalName)
                        .then(function updateLastLoginForUser(userFromPrincipal) {
                            if (userFromPrincipal) {
                                // No need to wait, just use the user as it was previously retrieved
                                // If the last_login info is required in the future, updateLastLogin could return the user.
                                getUserService().updateLastLogin(userFromPrincipal._id);
                            }
