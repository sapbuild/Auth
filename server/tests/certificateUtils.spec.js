...
describe('Certificate utils', function () {
    ...
    describe('getEmail', function () {
        it('should retrieve the subject email attribute', function () {
            expect(certificateUtils.getEmail(cert1)).to.equal('Alice@example.com');
            expect(certificateUtils.getEmail(cert2)).to.equal('Alice@example.com');
            expect(certificateUtils.getEmail(cert3)).to.equal('Alice@example.com');
            expect(certificateUtils.getEmail(cert4)).to.equal('Alice@example.com');
            expect(certificateUtils.getEmail(cert5)).to.equal('Alice@example.com');
        });
    });
    ...
    describe('getPrincipalFromMailAndIssuer', function () {
        it('should build a principal attribute from a mail and an issuer', function () {
            expect(certificateUtils.getPrincipalFromMailAndIssuer('Alice@example.com', 'sapnetca')).to.equal('alice@example.com|sapnetca');
        });
    });
});
